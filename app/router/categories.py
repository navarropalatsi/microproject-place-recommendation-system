...
def get_all_categories(dao: CategoryDAO = Depends(get_category_dao), skip: int = 0, limit: int = 25) -> list[SingleCategory]:
    return dao.all(skip=skip, limit=limit, order="ASC")

@router.get("/{name}", description="Get a single category", response_model=SingleCategory)
def find_category_by_name(name: str, dao: CategoryDAO = Depends(get_category_dao)) -> SingleCategory:
    return dao.find(name=name)

@router.post("", description="Create a new category", response_model=SingleCategory)
def create_category(data: SingleCategory, dao: CategoryDAO = Depends(get_category_dao)) -> SingleCategory:
    return dao.create(name=data.name)

@router.delete("/{name}", description="Delete a category", response_model=bool)
def delete_category(name: str, dao: CategoryDAO = Depends(get_category_dao)) -> bool:
