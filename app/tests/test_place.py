...
def test_create_places_and_list_them(client):
    size = 10
    places = []
    for i in range(size):
        place = get_place_faker()
...
def test_can_update_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
...
def test_can_delete_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
...
def test_cannot_delete_non_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
...
def test_can_attach_a_feature_to_an_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
...
def test_can_detach_a_feature_to_an_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
    ...
    response = client.delete(
        "/places/" + place["placeId"] + "/has-not/" + feature["name"]
    )
    assert response.status_code == 200
    place: SinglePlaceExtended = response.json()
    for f in place["features"]:
...
def test_can_attach_a_category_to_an_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
...
def test_can_detach_a_category_to_an_existing_place(client):
    response = client.get("/places?limit=1")
    assert response.status_code == 200
    place = response.json()[0]
    ...
    response = client.delete(
        "/places/" + place["placeId"] + "/is-not-in/" + category["name"]
    )
    assert response.status_code == 200
    place: SinglePlaceExtended = response.json()
    for f in place["categories"]:
