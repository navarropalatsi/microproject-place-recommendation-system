...
def test_create_users_and_list_them():
    size = 10
    users = []
    for i in range(size):
        user = get_user_faker()
...
def test_cannot_create_existing_user():
...
def test_update_existing_user():
    ...
    user = response.json()[0]
    new_born = faker.date_of_birth(tzinfo=pytz.UTC, minimum_age=20, maximum_age=40).isoformat()
    new_gender = 'm' if user['gender'] == 'f' else 'f'
    response = client.put("/users/" + user['userId'], json={
        "userId": user['userId'],
        "gender": new_gender,
        "born": new_born
    })
...
def test_cannot_delete_non_existing_user():
    response = client.get("/users?limit=1")
    assert response.status_code == 200
    user = response.json()[0]
